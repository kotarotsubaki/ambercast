...
| --- | --- | --- |
| TP2 changes target config from `targets.<name>.browser` to `targets.<name>.executor`, removes `RunCaseOutcome.engine`, adds `sessions[name].executor` to the report contract, adds the `EXECUTOR_UNSUPPORTED` report error code, and renames the `BROWSER_LAUNCH_FAILED` reason by changing its `engine` prefix to `executor` (`executor-unregistered`); the Plan schema is unchanged | repo:src/core/config/schema.ts, repo:src/report/schema.ts, repo:src/report/error-mapping.ts | Remove the old target browser key when explicit executor selection is unnecessary; otherwise migrate it to `executor: { kind: "playwright", browser: "chromium" }` (`executor.browser`). The loader rejects the old key with `ConfigInvalidError`. Report consumers must handle the removed `RunCaseOutcome.engine`, added `sessions[name].executor`, new `EXECUTOR_UNSUPPORTED` code, and renamed reason; Plan regeneration is not required by this schema change. |
