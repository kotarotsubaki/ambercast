...
Provider-supplied anchor and column coordinates bind each criterion to one locally resolved prompt excerpt; local resolution against a four-coordinate `InstructionSourceSpan` makes the committed attribution precise, while the provider's `citation` is only a checksum confirming that resolution, never a join key.  [repo:src/usecases/instruction-coverage-policy.ts:369-399]
