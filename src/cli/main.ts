 ...
 * affect the `status` column or the JSON output path. A Stage 3
 * secret-set rejection adds one result-local remediation line immediately
 * after its row, rather than entering the error channel: healing must leave
 * artifacts unchanged until regeneration receives fresh consent (SPEC-C3-2).
 */
