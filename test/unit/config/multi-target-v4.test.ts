...
const config = { $schema: 'https://ambercast.dev/schema/config.json', targets: { app: { baseUrl: 'https://example.test', browser: 'chromium', surface: 'web', description: 'Customer browser' } }, defaultTarget: 'app' };
...
describe('Target config for Plan v4', () => {
  it('accepts a web surface and description without publishing them as browser choice', () => {
    ...
    expect(RawConfig.safeParse({ ...config, targets: { app: { baseUrl: 'https://example.test', browser: 'chromium', description: 'Customer browser' } } }).success).toBe(true);
  });

  it('defaults omitted surface to web in resolved configuration', async () => {
    const storage = createInMemoryStorage();
    await storage.writeText(path, JSON.stringify({ ...config, targets: { app: { baseUrl: 'https://example.test', browser: 'chromium', description: 'Customer browser' } } }));
