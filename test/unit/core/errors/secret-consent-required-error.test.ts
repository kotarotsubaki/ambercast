...
import { SecretConsentRequiredError, type SecretConsentRequiredDetails } from '#core/errors/secret-consent-required-error.js';

describe('SecretConsentRequiredError', () => {
  it('preserves kind, base-class exit code, message, details, and cause', () => {
    const cause = new Error('cause'); const details = { reason: 'consent-required', secrets: [], hint: 'Configure secrets.allow before retrying.' } satisfies SecretConsentRequiredDetails;
