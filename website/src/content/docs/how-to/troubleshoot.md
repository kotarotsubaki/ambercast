...
| Heal reports `secret-set-changed` | A Stage 3 candidate changed the complete secret-name set, which heal cannot reattribute or consent to silently. | Run `ambercast generate --force <file>` and complete consent; the shipped CLI renders this literal Japanese hint verbatim: `秘匿値の構成が変わった。ambercast generate --force <file> で再生成し同意を取り直す` |
