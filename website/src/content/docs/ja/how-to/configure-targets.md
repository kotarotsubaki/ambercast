...
1. `{"$schema":"https://kotarotsubaki.github.io/ambercast/schemas/config.schema.json","targets":{"staging":{"baseUrl":"https://staging.example.test","executor":{"kind":"playwright","browser":"chromium"}},"admin":{"baseUrl":"https://admin.example.test"}},"defaultTarget":"staging"}` を記述します。これは公開されている設定スキーマURLであり、`targets` および `defaultTarget` は受け入れられる設定形式を持ちます。
