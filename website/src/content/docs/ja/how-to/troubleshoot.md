...
| heal が `secret-set-changed` を報告する | Stage 3 の候補がシークレット名の集合全体を変更しました。heal はこれを黙って再帰属・同意できません。 | `ambercast generate --force <file>` を実行して同意を完了します: `秘匿値の構成が変わった。ambercast generate --force <file> で再生成し同意を取り直す` |
