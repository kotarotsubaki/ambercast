...
3. シークレットが必要な場合にのみ、独立した行として `@ambercast-secret {{secrets.name}}` を記述します。パーサーはコードブロック外に置かれた完全な付与行のみを認識します。グラント行1つは1回の使用だけを認可するため、同じシークレットを複数回使用する場合は使用ごとに行を1つ繰り返します。
