...
heal は新しいシークレット同意を決定しません。Stage 2 の候補に無効なシークレット名、許可リスト違反、または環境変数の衝突がある場合、その修復を `secret-name-invalid` として拒否します。Stage 3 の完全 Plan 候補が論理シークレット名の集合全体を変更した場合、heal は `stage3Rejection.reason: "secret-set-changed"` として拒否し、Plan または Grounding を更新せず、次のヒントを表示します。
