...
`ambercast mcp` はバージョン 0.3.1 では実装されていません。パーサーが受け付けるコマンドは `generate`、`run`、`check`、`heal`、`init`、`view` のみであり、それ以外のコマンドは拒否されます。計画されている v2 サーバーインターフェースは、MCP 設計によって定義されています。
