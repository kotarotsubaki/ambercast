...
| report 3.5 → 3.6 | [src/report/schema.ts:42](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/report/schema.ts#L42) | Report consumer は、Plan バージョンから互換性を推論するのではなく、4つの completed heal-result ブランチすべてに追加された任意の `repairTrace` 診断フィールドを含む現在の `3.6` コントラクトを受け入れなければならない（MUST）。これはケースごとの Stage 1/2/3 の試行と結果の履歴を投影する（issue #383 / SPEC-9〜SPEC-12）。[src/report/schema.ts:42](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/report/schema.ts#L42) |
