...
`ambercast mcp` is not implemented in 0.3.1; the parser accepts only `generate`, `run`, `check`, `heal`, `init`, and `view`, and rejects any other command. Its planned v2 server interface is defined by the MCP design.
