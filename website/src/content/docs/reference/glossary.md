...
| `--target` | `--target` names a configured execution target for `generate`, `run`, `check`, and `heal`. Neither `init` nor `view` accepts it. | CLI parser / target resolver; [CLI overview](/ambercast/reference/cli/overview/#command-flag-matrix) | target definition itself |
