...
ambercast 解析器所公开实现的命令恰好为 `init`、`generate`、`run`、`check` 与 `heal`。本页为 AI Agent 提供调用这些命令时的动作边界、前提条件、副作用以及所需的批准规则，帮助您判断是否能够安全地调用相应命令。

## 命令契约 {#command-contract}

解析器公开实现的命令恰好为 `init`、`generate`、`run`、`check` 与 `heal`。其中，`check` 命令完全不依赖可变存储、AI 提供商或浏览器。对于 `heal` 命令，系统仅将计划与 grounding 伴生文件的修改缓冲至确认阶段；但在获得批准之前以及在 dry run 运行期间，其重放尝试可能会在用例运行目录中写入包含证据。
