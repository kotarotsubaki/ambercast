...
在 0.3.1 中，CLI 解析器所暴露的子命令包括 `init`、`generate`、`run`、`check`、`heal` 和 `view`。当前已实现的解析器并未暴露 MCP 子命令。
...
在当前实现的边界之外，项目规划中的文档涵盖了 `review`、`mcp`、baseline/restore、MCP 工具以及官方 skill。

在浏览器平台支持方面，Firefox 和 WebKit 计划在 1.0 达成。

相关链接：[ambercast review](/ambercast/zh-cn/reference/cli/review/)、[official skill](/ambercast/zh-cn/agents/official-skill/)、[MCP 服务器](/ambercast/zh-cn/agents/mcp-server/)
