...
1. 在单独的非代码提示词行中精确写入 `@ambercast-secret {{secrets.password}}`。授权解析器会接受完整匹配的行，并排除代码块、缩进代码和行内代码。一行授权仅授权一次使用；同一机密需要多次使用时，每次使用都重复一行。例如，先登录、登出后再次登录。将每个授权行紧邻放在描述其对应使用的指令之前并非硬性要求（参见[授权来源规则](/ambercast/zh-cn/spec/secrets/#grant-origin)），但这样做应当（SHOULD）能让这种一一对应关系对提供商更加明确，有助于避免引用无法解析。
