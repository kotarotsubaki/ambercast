...
始终从已安装 ambercast 的项目启动客户端，以便 `npx --no-install` 能解析该包；`--dir <path>` 仅在服务器启动后选择另一个 session root，不影响这一包解析过程。服务器提供 `ambercast_generate`、`ambercast_run`、`ambercast_check`、`ambercast_heal`、`ambercast_job_status` 和 `ambercast_job_cancel`。
