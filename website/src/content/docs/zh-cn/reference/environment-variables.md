...
| `AMBERCAST_DEBUG` | `main.ts` 中的 bare catch；`stderr-progress-sink.ts` 中 run 的进度输出端 | 将崩溃原因的消息和堆栈输出到 stderr。 | 还会将 run 中未分类案例异常的分类名称、已脱敏消息和堆栈输出到 stderr；这些诊断可能包含源自请求的敏感文本。未设置、为空、严格等于 `0` 或严格等于 `false` 时不激活；其他情况均激活。比较区分大小写且不裁剪空白字符。 |
