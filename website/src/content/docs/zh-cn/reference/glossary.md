...
| `--target` | `--target` 为 `generate`、`run`、`check`、`heal` 指定命名的已配置执行目标。`init` 与 `view` 均不接受该选项。 | CLI 解析器 / 目标解析器；[CLI 概览](/ambercast/zh-cn/reference/cli/overview/#command-flag-matrix) | 目标定义本身 |
