...
| report | `https://kotarotsubaki.github.io/ambercast/schemas/report.v3.schema.json` | 3 (`schemaVersion: "3.0"`) | `ambercast report schema v3` | `Zod schema for the complete versioned output of a reporting command.` | 结构化报告外层信封（envelope）及其命令专属结果。 | 计划中（0.3.1 尚未实现） |
...
| 产物 | 当前状态 |
| --- | --- |
| 报告 JSON Schema | 目前尚未生成：生成器的详尽写入列表和 package 导出映射仅包含 plan、grounding 和 config。运行时报告仍带有 `schemaVersion: "3.0"`。 |
