...
| report | `https://kotarotsubaki.github.io/ambercast/schemas/report.v3.schema.json` | 3 (`schemaVersion: "3.6"`) | `ambercast report schema v3.0` | `Zod schema for the complete versioned output of a reporting command.` | 结构化报告外层信封（envelope）及其命令专属结果。 | 可用 |
...
| `report.schema.json` | 包含 Report `schemaVersion` 3.6 的结构化报告外层信封（envelope）及其命令专属结果。 | Zod `ReportEnvelope` 转换为 JSON Schema 2020-12。 | `ambercast/schema/report.json` → `./dist/schema/report.schema.json` |
...
- 重复的 Plan 步骤 ID 属于仅在 Zod 中生效的语义细化约束，因为 JSON Schema 2020-12 无法表达数组项之间投影字段的唯一性。
- 仅验证生成的 Plan JSON Schema 并不能确立重复步骤 ID 的唯一性；[符合性](/ambercast/zh-cn/spec/conformance/) 记录了这一组合约束边界。

## 报告 Schema {#report-schema}

`report.schema.json` 与其他 Schema 一同生成，并作为 `ambercast/schema/report.json` → `./dist/schema/report.schema.json` 导出。它验证包含 Report `schemaVersion` 3.6 的结构化报告外层信封（envelope）及其命令专属结果。
