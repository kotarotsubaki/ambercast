...
计划必须（MUST）仅使用 [值类型](/ambercast/zh-cn/spec/value-types/#shared-types) `SecretRef` 来表示机密值；连续的 `{{secrets.` 插值标记禁止出现在 `InterpolatableText` 中。[src/core/ir/schema.ts:31-33](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L31-L33) [src/core/ir/schema.ts:159](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L159)

已提交的 `fill-secret` 必须（MUST）恰好包含 `target: ElementRef` 和 `secretRef: SecretRef`。已提交 AI 步骤的可选 `secrets` 数组包含恰好为 `{ "ref": SecretRef }` 的 `AiStepSecretUse` 条目。两种已提交形式均不包含 provider 命名数据、引文数据或 prompt 源出处信息。[src/core/ir/schema.ts:396](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L396) [src/core/ir/schema.ts:489-504](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L489-L504) [src/core/ir/schema.ts:667-692](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L667-L692)

在运行时，`{{secrets.NAME}}` 仅通过 `AMBERCAST_SECRET_<NAME>` 解析：先将 `NAME` 中的每个 `.` 替换为 `_`，再转换为大写。解析后的值严禁（MUST NOT）进入 Plan。[src/core/secrets/env-var-name.ts:8-16](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/secrets/env-var-name.ts#L8-L16) [src/adapters/system/env-secrets-provider.ts:14-34](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/adapters/system/env-secrets-provider.ts#L14-L34)

## 同意与允许列表 {#grant-origin}

生成必须（MUST）先构建候选 Plan，再在持久化该 Plan 或其 Grounding 前决定同意。候选阶段在本地将每个 provider 提案解析为 `SecretName`，将其包装为 `SecretRef`，并报告每次使用；同意阶段只允许 `ambercast.config.json` 的 `secrets.allow` 中已有的名称，或用户已接受的名称。[src/usecases/generate.ts:1094-1155](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1094-L1155) [src/usecases/generate.ts:1405-1424](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1405-L1424) [src/core/config/schema.ts:78-84](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/config/schema.ts#L78-L84)

`SecretName` 是配置与同意使用的裸逻辑名称；`SecretRef` 是其 `{{secrets.NAME}}` Plan 包装。面向 provider 的 `fill-secret` 可以（MAY）改为携带 `secret?: SecretNameChoice`：`{ "allowedName": SecretName }` 请求复用已投影的允许列表名称，而 `{ "nameHint": SecretNameHint }` 提议新发现的名称。`SecretNameChoice` 是严格的互斥联合，`SecretNameHint` 为 `/^[a-z][a-z0-9_]{0,63}$/`。面向 provider 的 AI 机密使用采用相同 choice；若 provider 未提供 choice，则使用空的严格对象。每个已提交的 `SecretRef` 都由本地确定性命名产生，而不是由 provider 引用产生。[src/core/ir/schema.ts:83-115](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L83-L115) [src/core/ir/schema.ts:729-785](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L729-L785) [src/core/ir/schema.ts:813-833](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L813-L833) [src/usecases/secret-naming.ts:156-188](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/secret-naming.ts#L156-L188)

接受时，新接受的名称必须（MUST）合并到 `secrets.allow`，以便后续生成跳过对这些名称的进一步决定。`secrets.allow: "*"` 接受每个名称并绕过同意门。该通配符是有意设计的低摩擦、高风险逃生口：它无需逐名称审查便接受任意 AI 提议的机密名称，因此只能（MUST）在理解该风险的情况下使用。[src/usecases/generate.ts:1423-1482](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1423-L1482) [src/config/write-secrets-allow.ts:52-82](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/config/write-secrets-allow.ts#L52-L82)

使用 `consentMode: "forbid"` 时，生成必须（MUST）返回包含提议 Plan 和机密使用的 `candidate` 结果，且严禁（MUST NOT）写入工件或请求同意。如果同意被拒绝，或因终端为非交互式而不可用，生成必须（MUST）以 `SECRET_CONSENT_REQUIRED` 失败，且严禁（MUST NOT）静默持久化候选项。[src/usecases/generate.ts:1251-1257](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1251-L1257) [src/usecases/generate.ts:1405-1420](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1405-L1420) [src/usecases/generate.ts:1426-1493](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1426-L1493) [src/runtime/secret-consent.ts:293-321](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/runtime/secret-consent.ts#L293-L321)

provider 接收的是已投影／协商的机密名称，而非机密值。名称本身可能具有描述性（例如 `stripe_api_key`），因此这是可接受的 provider 披露风险；实现严禁（MUST NOT）将其表示为值披露。[src/usecases/generate.ts:1020-1027](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/generate.ts#L1020-L1027) [src/core/ir/schema.ts:83-115](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/core/ir/schema.ts#L83-L115)

Heal 严禁（MUST NOT）作出新的同意决定。如果其 Stage 3 完整 Plan 候选更改了逻辑机密名称集合，它必须（MUST）恢复其 overlay，并返回带有 `stage3Rejection.reason: "secret-set-changed"`、`added` 和 `removed` 的 `secret-set-rejected` 结果分支；其用例结果为 `commit: undefined`。已发布 CLI 会逐字呈现该分支的本地化提示：`秘匿値の構成が変わった。ambercast generate --force <file> で再生成し同意を取り直す`。当本地命名、允许列表验证或环境变量冲突阻止允许的替换 Plan 时，Stage 2 使用不同的 `secret-name-invalid` 拒绝原因。[src/usecases/heal.ts:191-205](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/heal.ts#L191-L205) [src/usecases/heal.ts:856-878](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/heal.ts#L856-L878) [src/usecases/heal.ts:1012-1019](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/heal.ts#L1012-L1019) [src/usecases/heal.ts:1296-1314](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/usecases/heal.ts#L1296-L1314) [src/ports/system.ts:8-33](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/ports/system.ts#L8-L33) [src/cli/main.ts:335-354](https://github.com/kotarotsubaki/ambercast/blob/v0.3.1/src/cli/main.ts#L335-L354)
