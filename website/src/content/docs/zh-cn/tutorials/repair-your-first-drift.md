...
2. 在触发失败运行之前，请确认 runs 目录的保留策略（retention policy）以及授权读取者。一次完整的 `run` 执行会尝试向该目录持久化写入 `report.json`，并可能写入失败截图（尽力而为：在检测到机密信息、截图捕获失败或存储写入失败时将被忽略）。接着，进行一项会影响定位器的 UI 变更，并运行 `npx ambercast run tests/ambercast/<name>.test.md`。在尝试修复之前，观察并确认测试已失败。

→ **预期现象**：默认的失败关闭行为会将定位匹配缺失（grounding miss）直接判定为失败，而不是触发 AI 重新定位。这样可以确保在获得显式自愈批准之前，受版本控制的 grounding 不会被提前重写。只有有意进行实时重新定位时才传入 `--resolve`；默认的 `grounding.localWriteBack: "auto"` 会持久化重新解析后的指纹，且运行可能直接通过。
