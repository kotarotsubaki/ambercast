...
describe('documentation sidebar', () => {
  ...
  it('exports every sidebar slug exactly once and keeps REFERENCE direct pages before CLI records', () => {
    const flatten = (items: Array<{ slug?: string; items?: Array<{ slug: string }> }>) => items.flatMap((item) => item.slug ? [item.slug] : item.items!.map(({ slug }) => slug));
    const sidebarSlugs = sidebar.flatMap(({ items }) => flatten(items));

    expect(Array.isArray(orderedPages)).toBe(true);
    expect(orderedPages.map(({ slug }) => slug)).toHaveLength(sidebarSlugs.length);
    expect(new Set(orderedPages.map(({ slug }) => slug))).toEqual(new Set(sidebarSlugs));
    const reference = orderedPages.filter(({ groupLabel }) => groupLabel === 'REFERENCE');
    const firstCli = reference.findIndex(({ subgroupLabel }) => subgroupLabel === 'CLI');
    expect(firstCli).toBeGreaterThan(0);
    expect(reference.slice(0, firstCli).every(({ subgroupLabel }) => subgroupLabel === null)).toBe(true);
    expect(reference.slice(firstCli).every(({ subgroupLabel }) => subgroupLabel === 'CLI')).toBe(true);
  });
